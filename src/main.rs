...
fn find_distances(
    ...
    &coordinates.par_iter().for_each(|destination_coordinate| {
        let end_node = nodes_on_graph.get(&destination_coordinate.node.unwrap().id).unwrap().clone();
        let res = astar(&graph, origin_node, |finish| finish == end_node, |e| *e.weight(), |_| 0.);
